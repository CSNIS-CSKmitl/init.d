...
{#if consoleTarget}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/80 backdrop-blur-sm p-4 sm:p-6 transition-all duration-300">
		<div class="flex h-[80vh] w-full max-w-5xl flex-col rounded-xl border border-app bg-surface shadow-2xl overflow-hidden transition-all duration-300 transform scale-100">
			<!-- Modal Header -->
			<header class="flex items-center justify-between border-b border-app bg-elevated px-4 py-3 sm:px-6">
				<div class="flex items-center gap-2.5">
					<Terminal class="h-4 w-4 text-accent animate-pulse" />
					<div class="min-w-0">
						<h3 class="font-mono-app text-sm font-semibold text-app truncate">
							{consoleTarget.hostname} Console
						</h3>
						<p class="font-mono text-[10px] text-muted-app uppercase tracking-wider">
			...
			<!-- Modal Body (Iframe) -->
			<div class="relative flex-1 bg-zinc-950 flex items-center justify-center p-1">
				{#if consoleLoading}
					<div class="flex flex-col items-center gap-3 text-center p-8">
						<RefreshCw class="h-8 w-8 text-accent animate-spin" />
						<p class="font-mono text-xs uppercase tracking-widest text-secondary-app animate-pulse">// AUTHORIZING CONSOLE SESSION...</p>
					</div>
				{:else}
					{#if consoleError}
						<div class="max-w-md text-center p-8 space-y-4">
							...
							<p class="text-xs text-muted-app font-mono leading-relaxed">
								Failed to establish connection to the Proxmox console. Please make sure the instance is running and the hypervisor is online.
							</p>
